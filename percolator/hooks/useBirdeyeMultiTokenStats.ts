...
export function useBirdeyeMultiTokenStats(addresses: string[]) {
  const [statsMap, setStatsMap] = useState<Record<string, MultiTokenStats>>({});
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        console.error("MULTI TOKEN_STATS WS parse error:", e);
        setError(e.message);
      }
    };

    ws.onerror = (e) => {
      console.error("MULTI TOKEN_STATS WS error:", e);
  ...
  return { statsMap, connected, error };
}
