...
impl RiskEngine {
    ...
    pub fn keeper_crank(
        ...
        while accounts_processed < ACCOUNTS_PER_CRANK && slots_scanned < MAX_ACCOUNTS {
            ...
            if is_occupied {
                ...
                let _ =
                    self.settle_maintenance_fee_best_effort_for_crank(idx as u16, now_slot);
                ...
                if !force_realize_active && liq_budget > 0 {
                    ...
                    if !self.accounts[idx].position_size.is_zero() {
                        ...
                        if equity == 0 || is_dust {
                            ...
                            self.lifetime_force_realize_closes = self
                    ...
                    if !self.accounts[idx].position_size.is_zero() {
                            ...
                            .is_ok()
                        {
                                ...
                                .is_ok()
                            {
                                ...
                                force_realize_budget =
                                    force_realize_budget.saturating_sub(1);
                                self.lifetime_force_realize_closes = self
                                    .lifetime_force_realize_closes
                                    .saturating_add(1);
                            } else {
                ...
                if self.accounts[idx].is_lp() {
                    let abs_pos = self.accounts[idx].position_size.unsigned_abs();
                    self.lp_max_abs_sweep =
                        self.lp_max_abs_sweep.max(U128::new(abs_pos));
                }
            }
    ...
    pub fn liquidate_at_oracle(
        ...
        self.insurance_fund.balance =
            self.insurance_fund.balance.saturating_add_u128(U128::new(pay));
    ...
    pub fn update_warmup_slope(&mut self, idx: u16) -> Result<()> {
        ...
        let slope = if self.params.warmup_period_slots > 0 {
            ...
            if avail_gross > 0 { core::cmp::max(1, base) } else { 0 }
        } else {
    ...
    pub fn withdraw(
        ...
        if !position_size.is_zero() {
            let position_notional =
                mul_u128(saturating_abs_i128(position_size.get()) as u128, oracle_price as u128)
                    / 1_000_000;
        ...
        // Regression assert: after settle + withdraw, negative PnL should have been settled immediately
        #[cfg(any(test, kani))]
        debug_assert!(
    ...
    pub fn is_above_margin_bps_mtm(&self, account: &Account, oracle_price: u64, bps: u64) -> bool {
        ...
        let position_value =
            mul_u128(saturating_abs_i128(account.position_size.get()) as u128, oracle_price as u128)
                / 1_000_000;
    ...
    fn liq_priority_score(&self, a: &Account, oracle_price: u64) -> u128 {
        ...
        let pos_value =
            mul_u128(saturating_abs_i128(a.position_size.get()) as u128, oracle_price as u128)
                / 1_000_000;
    ...
    pub fn execute_trade<M: MatchingEngine>(
        ...
        let user_old_avail = {
            ...
            if pnl > 0 {
                (pnl as u128).saturating_sub(self.accounts[user_idx as usize].reserved_pnl as u128)
            } else {
                0
            }
        };
        let lp_old_avail = {
            ...
            if pnl > 0 {
                (pnl as u128).saturating_sub(self.accounts[lp_idx as usize].reserved_pnl as u128)
            } else {
                0
            }
        };
        ...
        if new_lp_position != 0 {
            ...
            let lp_crosses_zero =
                (old_lp_pos > 0 && new_lp_position < 0)
                    || (old_lp_pos < 0 && new_lp_position > 0);
        ...
        self.insurance_fund.balance =
            U128::new(add_u128(self.insurance_fund.balance.get(), fee));
        ...
        let new_oi =
            saturating_abs_i128(new_user_position) as u128 + saturating_abs_i128(new_lp_position) as u128;
        if new_oi > old_oi {
            self.total_open_interest =
                self.total_open_interest.saturating_add(new_oi - old_oi);
        } else {
            self.total_open_interest =
                self.total_open_interest.saturating_sub(old_oi - new_oi);
        }
        ...
        if new_lp_abs > old_lp_abs {
            self.lp_sum_abs =
                self.lp_sum_abs.saturating_add(new_lp_abs - old_lp_abs);
        } else {
            self.lp_sum_abs =
                self.lp_sum_abs.saturating_sub(old_lp_abs - new_lp_abs);
        }
    ...
    pub fn settle_warmup_to_capital(&mut self, idx: u16) -> Result<()> {
        ...
        if pnl > 0 {
            ...
            let new_avail = if new_pnl > 0 {
                (new_pnl as u128)
                    .saturating_sub(self.accounts[idx as usize].reserved_pnl as u128)
            } else {
                0
            };
